...
class BadDebtPipelineEndToEndTests(unittest.TestCase):
    ...
    def test_benchmark_passes_three_times_with_expected_results(self) -> None:
        for _ in range(3):
            ...
            self.assertIn("art. 89a ust. 3 ustawy VAT", result.answer)
            self.assertIn("art. 89a ust. 2 pkt 3 lit. a ustawy VAT", result.answer)
            self.assertIn("art. 89a ust. 2a ustawy VAT", result.answer)
            self.assertIn("art. 18f ust. 5 ustawy CIT", result.answer)
